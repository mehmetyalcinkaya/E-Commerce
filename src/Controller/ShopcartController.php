...
class ShopcartController extends AbstractController
{
    ...
    {
        $this->denyAccessUnlessGranted('IS_AUTHENTICATED_FULLY'); //LOGİN KONTROLÜ
        $user=$this->getUser();
        //dump($user);
        //die();
       // echo $user->getid();
        ...
        $sql= "SELECT p.image,p.title,p.sprice ,s.*
        ...
        return $this->render('shopcart/index.html.twig', ['shopcarts' => $shopcarts]);
    }
    ...
    public function del(Request $request, Shopcart $shopcart): Response
    {
        ...
        $em->remove($shopcart);
        $em->flush();
        $this->addFlash('info',' Ürün sepetten silindi');
        return $this->redirectToRoute('shopcart_index');
