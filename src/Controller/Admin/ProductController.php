...
class ProductController extends AbstractController
{
    ...
    public function index(ProductRepository $productRepository): Response
    {
        ...
        $sql= "SELECT * FROM product p INNER JOIN category ON p.category_id=category.id";
        $statement=$em->getConnection()->prepare($sql);
        //$statement->bindValue('pid',$parent);
        $statement->execute();
        $categoryname=$statement->fetchAll();
    ...
    public function iupdate(Request $request, $id, Product $product): Response
    {
        ...
        try {
            $file->move(
                $this->getParameter('images_directory'), //Service de tanımladığımız yol
                $fileName
            );
